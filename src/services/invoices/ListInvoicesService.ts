...
export default class ListInvoicesService {
  ...
  public async execute(client_id: string): Promise<IInvoiceSankhya[]> {
    ...
    const invoices = await Promise.all(invoicesToMap.map(async (invoiceCurrent: any) => {
      const valuesInvoices = Object.entries(invoiceCurrent).sort(order).map(b => b[1]).map((c: any) => c.$);

      let invoice: IInvoiceSankhya = {} as IInvoiceSankhya;

      await Promise.all(fields.map((field: any, index: number) => {
        invoice[field.name as keyof IInvoiceSankhya] = valuesInvoices[index];
        return field;
      }));
