...
class NorthSankhyaCloudRequest {
  ...
  public async initMgeSession() {
    const responseToken = await axios.post(
      'https://north.sankhyacloud.com.br/mge/service.sbr?serviceName=MobileLoginSP.login&outputType=json',
      {
    ...
    console.log(
      ...
      responseToken.headers['set-cookie'],
    );

    this.jsessionid = responseToken.data.responseBody.jsessionid.$;
    this.cookie = responseToken.headers['set-cookie']?.length
      ? responseToken.headers['set-cookie'][0]
      : undefined;
  }
